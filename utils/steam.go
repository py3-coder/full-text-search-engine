...
func stepOneB_a(body []byte) []byte {
...
func stepOneB(body []byte) []byte {
	if bytes.HasSuffix(body, []byte("eed")) {
		...
		if hasVowel(body[:len(body)-2]) {
			return stepOneB_a(body[:len(body)-2])
		}
	} else if bytes.HasSuffix(body, []byte("ing")) {
		if hasVowel(body[:len(body)-3]) {
			return stepOneB_a(body[:len(body)-3])
		}
	}
	return body
}

func stepOneC(body []byte) []byte {
...
func Stem(s string) string {
	...
	if len(word) > 2 {
		return string(stepFiveB(stepFiveA(stepFour(stepThree(stepTwo(stepOneC(stepOneB(stepOneA(word)))))))))
	}
	return string(word)
}
